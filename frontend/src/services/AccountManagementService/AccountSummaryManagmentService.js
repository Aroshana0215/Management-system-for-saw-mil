...
export const newAccountSummary = async(accountSummaryData) => {
    try {
        const docRef = await addDoc(collection(db, "accountSummary"), accountSummaryData);
        console.log("New accountSummary entered into the system with ID: ", docRef.id);
        return docRef.id;
    } catch (error) {
...
export const updateAccountSummary = async (accountSummaryId, accountSummaryData ) => {
...
export const getaccountSummaryById =  async (accountSummaryId) => {
    try {
        ...
        if (accountSummarySnapshot.exists()) {
            const accountSummary = { id: accountSummarySnapshot.id, ...accountSummarySnapshot.data() };
...
export const getActiveAccountSummaryDetails = async () => {
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            const accountSummaryDetails = { id: docSnapshot.id, ...docSnapshot.data() };
