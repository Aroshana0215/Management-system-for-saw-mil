...
export const newDependant = async(employDependentData) => {
    try {
        const docRef = await addDoc(collection(db, "employDependent"), employDependentData);
        console.log("New employDependent entered into the system with ID: ", docRef.id);
        return docRef.id;
    } catch (error) {
...
export const updateemployDependent = async (employDependentId, employDependentData ) => {
    try {
...
export const getemployDependentById =  async (employDependentId) => {
    try {
        ...
        if (employDependentSnapshot.exists()) {
            const employDependent = { id: employDependentSnapshot.id, ...employDependentSnapshot.data() };
