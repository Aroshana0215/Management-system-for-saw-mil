...
const EmployeeList = () => {
  ...
  const columns = [
    ...
    { field: "phoneNo", headerName: "Phone No", width: 120 },
    {
      ...
      sortable: false,
      renderCell: ({ row }) => {
        const salaryPerDay = row?.salaryPerDay;
        const salaryPerMonth = row?.salaryPerMonth;

        const hasDay =
          salaryPerDay !== null &&
          salaryPerDay !== undefined &&
          String(salaryPerDay).trim() !== "";

        const valueToShow = hasDay ? salaryPerDay : salaryPerMonth;

        if (valueToShow === null || valueToShow === undefined || String(valueToShow).trim() === "") {
          return "-";
        }

        return `${valueToShow}.00`;
      },
    },
    { field: "createdBy", headerName: "Created By", width: 120 },
    {
      ...
      renderCell: ({ row }) => (
        <Box sx={{ paddingTop: "5px", paddingLeft: "20px" }}>
          <Avatar
            src={row.employeeImage || "/default-profile.png"}
            alt="Employee"
    ...
    },
    {
      ...
      renderCell: ({ row }) => {
        ...
        if (row.status === "A") {
          chipProps = {
            ...chipProps,
            sx: { borderColor: "#66BB6A", color: "#1B5E20", backgroundColor: "#C8E6C9" },
          };
        } else {
          chipProps = {
            ...chipProps,
            sx: { borderColor: "#E57373", color: "#C62828", backgroundColor: "#FFCDD2" },
          };
        }
            ...
            <Tooltip title="Make Payment">
              <span>
                <IconButton
                  ...
                  size="medium"
                  disabled={isInactive}
                  sx={{ "&:hover": { backgroundColor: "#C8E6C9" } }}
                >
                  <PaymentIcon />
                </IconButton>
              </span>
  ...
  const handleSearch = () => {
    ...
    const filteredData = categories.filter((category) =>
      Object.values(category).some((value) => String(value).toLowerCase().includes(lowercasedGeneralQuery))
    );
